...
def create_sound_filenames():
    ...
    global PAUSING_BETWEEN_AUDIO_FILE_DURATION_FASTER,PAUSING_BETWEEN_AUDIO_FILE_DURATION_SLOWER, INTRO_WAV, SET_DAY_WAV, SET_HOUR_WAV, YOU_SELECTED_WAV, NUMBERS_WAV, DAYS_WAV, AM_WAV, PM_WAV, PRESS_AGAIN_TO_QUIT_WAV,\
        EXITING_PROGRAM_WAV, PRESS_TO_QUIT_WAV, EXITING_PROGRAM_WAV_DURATION, EXITING_PROGRAM_WAV_DURATION, TMP_FILE_WAV, DAY_SET_WAV, HOUR_SET_WAV, SET_MINUTES_WAV, MINUTES_SET_WAV, OH_WAV
    ...
    # Included all 60 numbers for minutes in program
    ...
    SET_MINUTES_WAV = my_files_path + "Set_minutes.wav"
    MINUTES_SET_WAV = my_files_path + "Minutes_set.wav"
    ...
    PAUSING_BETWEEN_AUDIO_FILE_DURATION_SLOWER = 2.38 #  needs to be long enough for the sound to play "saturday at 1259pm "
    PAUSING_BETWEEN_AUDIO_FILE_DURATION_FASTER = 1.6 # faster for saying options directly after key is pressed
...
################################################################################
# Written by Anthony Hornof
# Talaba P. added a few sounds to test
def verify_sound_filenames():
    ...
    sound.Play(OH_WAV)
    sound.Play(MINUTES_SET_WAV)
    sound.Play(DAY_SET_WAV)
    sound.Play(HOUR_SET_WAV)
    sound.Play(PRESS_TO_QUIT_WAV)
...
def create_menu_globals():
    ...
    global HELP_KEY, REPEAT_KEY, SET_KEY, FORWARD_KEY, BACKWARD_KEY, QUIT_KEY, MINIMAL_HELP_STRING, CURRENT_TIME, CURRENT_DAY , CURRENT_HOUR, CURRENT_MINUTE
    ...
    CURRENT_HOUR = 1      # the current hour is set. (hour should never be zero)
             ...
             # minutes == true
                ...
                if am == True:
                    ...
                    if(CURRENT_MINUTE < 10 and CURRENT_MINUTE > 0):
                        ...
                        # Concatenate three audio files to generate the message.
                        sound.combine_wav_files(TMP_FILE_WAV, DAYS_WAV[CURRENT_DAY], NUMBERS_WAV[CURRENT_HOUR], OH_WAV, NUMBERS_WAV[CURRENT_MINUTE], AM_WAV, EXITING_PROGRAM_WAV)
                        sound.Play(TMP_FILE_WAV)

                    # if the minutes are equal to zero then we should not say the minutes, example 1:00 = "1 am" or "1 pm"
                    elif(CURRENT_MINUTE == 0):

                        sound.combine_wav_files(TMP_FILE_WAV, DAYS_WAV[CURRENT_DAY],
                                    NUMBERS_WAV[CURRENT_HOUR], AM_WAV)
                        sound.Play(TMP_FILE_WAV)
                        ...
                        sound.combine_wav_files(TMP_FILE_WAV, DAYS_WAV[CURRENT_DAY],
                                    NUMBERS_WAV[CURRENT_HOUR], NUMBERS_WAV[CURRENT_MINUTE], AM_WAV, EXITING_PROGRAM_WAV)
                        sound.Play(TMP_FILE_WAV)
                ...
                # if pm is true and am is false
                    ...
                    if(CURRENT_MINUTE < 10 and CURRENT_MINUTE > 0):
                        ...
                        sound.combine_wav_files(TMP_FILE_WAV, DAYS_WAV[CURRENT_DAY],
                                    NUMBERS_WAV[CURRENT_HOUR], OH_WAV, NUMBERS_WAV[CURRENT_MINUTE], PM_WAV)
                        sound.Play(TMP_FILE_WAV)
                    ...
                    # if the minutes are equal to zero then we should not say the minutes, example 1:00 = "1 am" or "1 pm"
                        ...
                        sound.combine_wav_files(TMP_FILE_WAV, DAYS_WAV[CURRENT_DAY],
                                    NUMBERS_WAV[CURRENT_HOUR], NUMBERS_WAV[CURRENT_MINUTE], PM_WAV)
